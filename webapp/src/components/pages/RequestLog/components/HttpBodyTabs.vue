...
const iframeSrc = computed(() => {
  ...
  return (
    ...
    `/${props.body?.hash}?content_type=${encodeURIComponent(props.body?.contentType ?? '')}`
  )
})
  ...
  <div class="ps-3">
      ...
      <div class="col-12" v-if="expandedState">
        <ul class="nav nav-tabs">
          <li class="nav-item" v-for="item in tabs" :key="item.key" @click="activeTab = item.key">
            ...
            <button
              v-else
              class="nav-link bg-secondary placeholder text-secondary"
              href="#"
              v-text="item.title"
            />
          </li>
    ...
    <div class="row" v-if="expandedState">
      <div class="col-12">
        ...
        <iframe
          v-else-if="activeTab === TAB_KEY_BROWSER_PREVIEW"
          :src="iframeSrc"
          class="w-100"
          height="400"
        />
