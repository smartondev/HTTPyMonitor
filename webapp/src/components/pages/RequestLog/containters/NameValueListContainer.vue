...
const { expandedState, toggleExpanded } = useExpand()

const placeholderList = ref<NameValuePairs>([])
onMounted(() => {
  const placeholderValueCount: number = randomInt(
    props.placeholderValueMin,
    props.placeholderValueMin
  )
  placeholderList.value = Array.from({ length: placeholderValueCount }, () => ({
    name: 'col-' + randomInt(1, 3),
    value: 'col-' + randomInt(2, 5)
  }))
})

const usePlaceholder = computed<boolean>(() => {
  return props.placeholder && props.list === undefined
})

const itemList = computed<NameValuePairs>(() => {
  ...
  <div class="container-fluid mb-3 ms-0 ps-0 placeholder-glow">
    <div class="row">
      <div class="col-lg-12 cursor-pointer" @click="toggleExpanded">
        <span class="fs-6 fw-bold me-2">{{ props.title }}</span>
    ...
    >
      <div class="row gap-2 font-monospace ps-3" v-for="(item, idx) in itemList" :key="idx">
        <div class="col-12 fs-90" v-if="usePlaceholder">
          <span class="placeholder me-1 bg-primary" :class="[item.name]" /><span
            class="placeholder"
            :class="[item.value]"
          />
        </div>
